...
function indekseGoreFenomen(inx,nm) {
  const fenx = inx[nm];
  const prx = fenx.profile;
  const wrf = nm + "." +``+ "indekste bulunan fenomen:" +``+ prx;
  return wrf;
}
...
function profilListesi(prf) {
  let npr = [];
  for (let index = 0;index < prf.length; index++){
    let lst = prf[index].profile;
    npr.push(lst);
  }
  return npr;
}
...
function fenomenSil(fnm,del) {
  let ary = fnm;
  delete ary[del];
  const res = ary.filter(element => {
...
function fenomenEkle(add,num,pro,fol,pts,plt) {
  let arry = add;
function addfenomen(){
const NeObj = {
   number: num,
...
function enFenomenler(fn) {
  let cal = [];
  for (let index = 0; index < fn.length; index++){
    if (fn[index].followers > 100000000){
      cal.push(fn[index].profile);
    }
  }
  return cal;
}
...
function fenomenGonderimSayisi(ary,pst) {
  for (let index = 0;index < ary.length; index++){
    let lst = ary[index].profile;
    if(lst === pst){
      let num = ary[index].posts;
      return num;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(arry, pltf) {

  let pl = [];
  for (let index = 0; index < arry.length; index++) {
    let lpt = arry[index].platform;
    if (lpt === pltf) {
      pl.push(arry[index]);
    }
  }
  pl.sort((s1, s2 ) => {return s2.posts - s1.posts ;});
  return pl[0].profile;
}
